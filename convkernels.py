...
class Conv(gpflow.kernels.Kernel):
    ...
    def _get_patches(self, X,flag):
        ...
        y=tf.cast(tf.reshape(patches, [tf.shape(X)[0], self.colour_channels * shp[1] * shp[2], shp[3]]),
                       float_type)
 #       tot_patches=y.get_shape().as_list()[1]
 #       tot_imgs=y.get_shape().as_list()[0] 
        tot_pat=int((((self.img_size[0] - self.patch_size[0])/(self.patch_size[0])) + 1) * (((self.img_size[1] - self.patch_size[1])/self.patch_size[1]) + 1))
        if flag==0:
            return y
        else: 
            for i in range(1000):
                tf.random_shuffle(y[i,:,:]) #tf.while_loop(y[i,:,:]=tf.random_shuffle(y[i,:,:]))            
            return y[:,0:int(tot_pat/10),:]

    def K(self, X, X2=None):
        Xp = self._get_patches(X,flag=1)
        Xp = tf.reshape(Xp, (-1, self.patch_len))
        Xp2 = tf.reshape(self._get_patches(X2,flag=1), (tf.shape(X2)[0], self.patch_len)) if X2 is not None else None
    ...
    def Kzx(self, Z, X):
        Xp = self._get_patches(X,flag=1)  # N x num_patches x patch_len
        Xp = tf.reshape(Xp, (-1, self.patch_len))
    ...
    def num_patches(self):
       return int(int((((self.img_size[0] - self.patch_size[0])/(self.patch_size[0])) + 1) * (((self.img_size[1] - self.patch_size[1])/self.patch_size[1]) + 1))/10) #(H-h+2P/s+1)x(W-w+2P/s+1), here taking S(stride length)=patch size , i.e 0% overlap.#????


    @gpflow.autoflow((float_type,))
    def compute_patches(self, X):
        return self._get_patches(X,flag=0)
...
class WeightedConv(Conv):
    def __init__(self, basekern, img_size, patch_size, colour_channels=1):
        Conv.__init__(self, basekern, img_size, patch_size, colour_channels)
        self.W = gpflow.params.Parameter(np.ones(self.num_patches,dtype=np.float32))

    def K(self, X, X2=None):
        Xp = self._get_patches(X,flag=1)
        Xp = tf.reshape(Xp, (-1, self.patch_len))
        Xp2 = None if X2 is None else tf.reshape(self._get_patches(X2,flag=1),
    ...
    def Kdiag(self, X):
        Xp = self._get_patches(X,flag=1)  # N x num_patches x patch_dim
