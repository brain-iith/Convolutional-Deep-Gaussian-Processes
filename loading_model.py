...
# log_file = open("logs/3layered.log","w")
...
def make_dgp(L):
    ...
    kernels = [ckern.WeightedConv(RBF(9, ARD=True), [28, 28], [3, 3])]
    for l in range(L-1):
        kernels.append(RBF(50))
...
param_dict = pickle.load(open( "models/wconv3.pkl", "rb" ))
m_dgp3.assign(param_dict)
print (m_dgp3.layers[0].kern)
...
# np.save("nlpp/wconv3",nlpp)
# np.save("acc/wconv3",accuracy)
# np.save("time/wconv3",times)
# param_dict = m_dgp3.read_trainables()
# f = open("models/wconv" + str(len(m_dgp3.layers)) + ".pkl","wb")
# pickle.dump(param_dict,f)
# f.close()
