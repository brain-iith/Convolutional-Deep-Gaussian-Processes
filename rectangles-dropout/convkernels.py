...
class WeightedConv(Conv):
    def __init__(self, basekern, img_size, patch_size, colour_channels=1):
        Conv.__init__(self, basekern, img_size, patch_size, colour_channels)
        self.W = gpflow.params.Parameter(np.ones(self.num_patches,dtype=np.float64))
...
class ConvRBF(Conv):
    def __init__(self, img_size, patch_size):
        ...
        # tf.conv2d does not have a float64 version implemented yet, so keep this bit of code locally float64.
...
class WeightedMultiChannelConvGP(Conv):
    def __init__(self, basekern, img_size, patch_size, colour_channels=1):
        Conv.__init__(self, basekern, img_size, patch_size, colour_channels)
        self.basekern.input_dim = np.prod(patch_size)
        self.W = gpflow.params.Parameter(np.ones((self.colour_channels, self.num_patches // self.colour_channels),dtype=np.float64))
