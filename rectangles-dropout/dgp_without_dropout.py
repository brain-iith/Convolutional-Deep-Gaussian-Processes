...
log_file = open("logs/dgp_without_dropout.log","w")
    ...
    with session.as_default():
        for _i in range(iterations):
            if(_i%iter_epochs==0):
                ...
                f = open("models/dgp_without_dropout" + str(len(model.layers)) + ".pkl","wb")
...
np.save("nlpp/dgp_without_dropout2",nlpp)
np.save("acc/dgp_without_dropout2",accuracy)
np.save("time/dgp_dwithout_ropout2",times)
param_dict = m_dgp2.read_trainables()
f = open("models/dgp_without_dropout" + str(len(m_dgp2.layers)) + ".pkl","wb")
pickle.dump(param_dict,f)
f.close()
...
print ("training time === ",time.time() - start)
# np.save("loss_wconv2",loss_val)
np.save("nlpp/dgp_without_dropout3",nlpp)
np.save("acc/dgp_without_dropout3",accuracy)
np.save("time/dgp_without_dropout3",times)
param_dict = m_dgp3.read_trainables()
f = open("models/dgp_without_dropout" + str(len(m_dgp3.layers)) + ".pkl","wb")
pickle.dump(param_dict,f)
f.close()
...
print ("training time === ",time.time() - start)
# np.save("loss_wconv2",loss_val)
np.save("nlpp/dgp_without_dropout4",nlpp)
np.save("acc/dgp_without_dropout4",accuracy)
np.save("time/dgp_without_dropout4",times)
param_dict = m_dgp4.read_trainables()
f = open("models/dgp_without_dropout" + str(len(m_dgp4.layers)) + ".pkl","wb")
pickle.dump(param_dict,f)
f.close()
